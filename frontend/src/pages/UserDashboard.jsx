...
import { Calendar, Clock, AlertTriangle, BookMarked, ArrowRight, RotateCcw, CheckCircle } from 'lucide-react';
  ...
  return (
    <Layout>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <Link
          to="/books"
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-sm font-semibold transition-all shadow-md shadow-indigo-600/10 shrink-0"
        >
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 rounded-2xl flex flex-col items-center justify-center text-center max-w-xl mx-auto border border-slate-800/80">
          ...
          <p className="text-slate-400 text-sm max-w-sm mb-6 leading-relaxed">
            You do not currently have any active rentals. Explore our books catalogue to borrow code and reference manuals!
          </p>
          <Link
            to="/books"
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold transition-all text-sm"
          >
            Browse Books
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rentals.map((rental) => {
            ...
            const isOverdue = rental.is_overdue || daysLeft < 0;

            return (
              <div
                key={rental.id}
                className={`glass-panel rounded-2xl overflow-hidden border flex flex-col transition-all duration-300 ${
                  isOverdue 
                    ? 'border-rose-500/30 hover:border-rose-500/50 shadow-lg shadow-rose-950/20' 
                    : 'border-slate-800 hover:border-slate-700/80 hover:shadow-lg hover:shadow-indigo-500/10'
                }`}
              >
                {/* Book Cover Banner */}
                {rental.book.image_url && (
                  <div className="h-28 w-full relative overflow-hidden select-none shrink-0 border-b border-slate-900/50">
                    <img 
                      src={rental.book.image_url} 
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-4 mb-2.5">
                      ...
                      {isOverdue ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 animate-pulse">
                          <Clock className="h-3 w-3" /> Active Loan
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-2 border-t border-slate-900 pt-4">
                    ...
                    <div className={`flex items-center gap-2.5 text-xs font-medium ${
                      ...
                      <Clock className={`h-4 w-4 shrink-0 ${isOverdue ? 'text-rose-500' : 'text-indigo-400'}`} />
                      <span>
                ...
                {/* Return Button Area */}
                <div className="p-4 bg-slate-900/40 border-t border-slate-800/60 mt-auto">
                  <button
                    onClick={() => handleReturn(rental.id)}
                    disabled={actionLoadingId === rental.id}
                    className="w-full inline-flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-bold bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800/80 disabled:opacity-50 transition-all focus:ring-1 focus:ring-indigo-500 outline-none"
                  >
